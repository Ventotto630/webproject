...
<div class="bottom">
    <c:if test="${currentPage > 1}">
        <a class="npaging" href="../../query-alllog.do?currentPage=${currentPage-1}&pageSize=${pageSize}">上一页</a>
    </c:if>
    <c:if test="${currentPage <= 1}">
        <span class="npaging">上一页</span>
    </c:if>

    <div style="width:520px;height:30px;overflow-x:auto;display: inline-block;float:left">
    <!-- 显示页码链接 -->
    <c:forEach var="i" begin="1" end="${totalPages}" step="1">
        <c:choose>
            ...
            <c:otherwise>
                <a class="npaging" href="../../query-alllog.do?currentPage=${i}&pageSize=${pageSize}">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>
    </div>
    ...
    <c:if test="${currentPage < totalPages}">
        <a class="npaging" href="../../query-alllog.do?currentPage=${currentPage+1}&pageSize=${pageSize}">下一页</a>
    </c:if>
    <c:if test="${currentPage >= totalPages}">
