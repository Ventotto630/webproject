...
<div class="bottom">
    <c:if test="${currentPage > 1}">
        <a class="npaging" href="../../findAllDepart.do?currentPage=${currentPage-1}&pageSize=${pageSize}">上一页</a>
    </c:if>
    <c:if test="${currentPage <= 1}">
    ...
    <c:forEach var="i" begin="1" end="${totalPages}" step="1">
        <c:choose>
            ...
            <c:otherwise>
                <a class="npaging" href="../../findAllDepart.do?currentPage=${i}&pageSize=${pageSize}">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>

    <c:if test="${currentPage < totalPages}">
        <a class="npaging" href="../../findAllDepart.do?currentPage=${currentPage+1}&pageSize=${pageSize}">下一页</a>
    </c:if>
    <c:if test="${currentPage >= totalPages}">
